...
def iterate_mf3(X : Tuple[Tensor, Tensor], params : Tuple[Tensor, Tensor, Tensor], alpha : float=1e-6, max_iter : int=2000, rho : float=0.) -> Tuple[Tensor, Tensor]:
    ...
    while True:
        ...
        field_v = vbias \
            + (mh @ weight_matrix.mT) \
            + (0.5 - mv) * (dmh @ weight_matrix2) \
            + (1/3 - 2 * dmv) * ((dmh * (0.5 - mh)) @ weight_matrix3)
        mv = rho * mv_prev + (1. - rho) * torch.sigmoid(field_v)
        eps1 = torch.abs(mv - mv_prev).max()
        eps2 = torch.abs(mh - mh_prev).max()
        if max(eps1, eps2) < alpha:
...
def iterate_mean_field(X : Tuple[Tensor, Tensor], params : Tuple[Tensor, Tensor, Tensor], order : int=2, batch_size : int=128, alpha : float=1e-6, verbose : bool=True, rho : float=0., max_iter : int=2000, device : torch.device=torch.device("cpu")) -> Tuple[Tensor, Tensor]:
